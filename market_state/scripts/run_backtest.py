...
then runs every fold of the walk-forward protocol with a **randomly
initialised** model (no training; ``--epochs`` defaults to 0). Setting
``--epochs`` > 0 does local supervised training, which is not the intended use
of this research benchmark.
...
def main() -> int:
    ...
    print(f"[backtest] features {feats.shape}, targets {sorted(targets.keys())}")
