#!/bin/bash
# mgsm-spot-train.sh -- launch ONE interruptible spot GPU box that (re)starts a
# resumable MGSM (Market Graph State Model) walk-forward training run and
# self-terminates only on a CLEAN finish. Called repeatedly by
# .github/workflows/mgsm-spot-keepalive.yml until the run writes its DONE
# sentinel to S3. See ../../EEG_Benchmarks/SPOT_TRAINING.md for the pattern.
#
#   market_state/scripts/mgsm-spot-train.sh --job mgsm \
#     --cmd 'python market_state/scripts/train.py --config market_state/configs/crypto.yaml \
#            --features /root/features.npz --device cuda --max-epochs 40 --patience 6'
#
# The script appends  --out /root/ckpt --s3-prefix s3://$BUCKET/checkpoints/<job> --resume
# so do NOT put those in --cmd.
#
# Behaviour on the box:
#   * DLAMI torch venv reused; market_state deps (minus torch) installed into it
#   * features.npz pulled from s3://$BUCKET/datasets/<job>/features.npz  (built
#     once locally with build_features.py -- NOT rebuilt per box)
#   * training auto-resumes: completed fold_XXX.pt are pulled from S3 and skipped
#   * clean finish of every fold -> run.py writes .../DONE, box terminates
#   * SIGTERM (spot)             -> trainer stops at the next fold boundary,
#     exits 42, NO DONE; AWS reclaims the box; keepalive launches the next one
#   * crash (rc!=0,42)           -> SNS notify, terminate, NO DONE
#
# Needs AWS creds for account 827938107865 (the GH workflow assumes
# alpaquero-gh-launcher via OIDC; a local admin shell also works).

set -euo pipefail

REGION=us-east-1
BUCKET=noshore-eeg-benchmarks-827938107865
REPO_URL=https://github.com/noshore5/Alpaquero.git
KEY=eeg-box
PROFILE_NAME=eeg-gpu           # reused: its role has S3 RW on the whole bucket
...
# g5 (A10G 24GB) first per the box-type decision; g4dn (T4 16GB) fallback --
# on 2026-09-07 g4dn was the only G family with us-east-1 spot stock. AZ 1e
# omitted (chronically capacity-poor for G family).
CANDIDATES=(
...
RUNNING=$(aws ec2 describe-instances --region $REGION \
  --filters "Name=tag:Project,Values=alpaquero" "Name=tag:spot-job,Values=$JOB" \
...
if ! aws s3 ls "$DATA_S3" --region $REGION >/dev/null 2>&1; then
  echo ">> $DATA_S3 missing -- build features locally and upload first:" >&2
  echo "   aws s3 cp market_state/data/features/features.npz $DATA_S3" >&2
  exit 1
fi
...
finish() {
  RC=\${RC:-1}
  aws s3 cp /var/log/mgsm-spot-train.log "\$LOG_S3/boot.log" || true
  ...
  if [ "\$RC" != 0 ] && [ "\$RC" != 42 ]; then
    aws sns publish --region $REGION --topic-arn "$SNS_TOPIC_ARN" \
      --subject "mgsm-spot-train \$JOB rc=\$RC" \
...
# market_state deps minus torch (DLAMI already has a CUDA torch)
grep -vE '^(torch|#|\s*\$)' market_state/requirements.txt > /tmp/reqs.txt || true
if [ -n "\$BASEPY" ]; then
  PY="\$BASEPY"
else
  echo "no cuda torch python found -- bare venv fallback (CPU only!)" >> /root/run.log
  python3 -m venv /root/venv && PY=/root/venv/bin/python
  "\$PY" -m pip install -U pip wheel >> /root/pip.log 2>&1 || true
  echo "torch>=2.0" >> /tmp/reqs.txt
fi
"\$PY" -m pip install -r /tmp/reqs.txt >> /root/pip.log 2>&1 \
  && echo "pip ok" >> /root/run.log || echo "pip FAILED (see pip.log)" >> /root/run.log
"\$PY" -c "import torch,numpy,scipy,pandas,mambapy;print('cuda',torch.cuda.is_available())" >> /root/run.log 2>&1

aws s3 cp "\$DATA_S3" /root/features.npz >> /root/run.log 2>&1
mkdir -p /root/ckpt
export PYTHONPATH=/root/repo/market_state/src
...
"\$PY" -u \$ARGS --out /root/ckpt --s3-prefix "\$PFX_S3" --resume >> /root/run.log 2>&1 &
TRAIN_PID=\$!
wait \$TRAIN_PID
RC=\$?
set -e
...
for pair in "${CANDIDATES[@]}"; do
  ...
  IID=$(aws ec2 run-instances --region $REGION \
    ...
    --tag-specifications "ResourceType=instance,Tags=[{Key=Project,Value=alpaquero},{Key=Name,Value=mgsm-spot-$JOB},{Key=spot-job,Value=$JOB}]" \
    --user-data "$UD" \
